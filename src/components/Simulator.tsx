  ...
  return (
    <main lang="ru" className="game-shell">
      ...
      <details className="game-reference">
        <summary>{scenario ? "Город после ваших решений — показатели районов" : "Город до решений — показатели районов"}</summary>
        {!scenario && <p className="my-3 text-sm">Показано исходное состояние. Итог появится после пяти допустимых решений.</p>}
        <CityBoard districts={(scenario ?? BASELINE).districts} synergiesApplied={(scenario ?? BASELINE).synergiesApplied} />
      </details>
    ...
    <section id="results" aria-label="Результат сценария" className="game-results flex flex-col gap-6">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          ...
          <Stat label={`Слабейшие районы · ${scenario.worstDistrictIds.map((id) => DISTRICTS.find((d) => d.id === id)?.name ?? id).join(", ")}`} value={scenario.worstDistrict.score.toFixed(2)} />
      ...
      <details className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <summary className="cursor-pointer font-semibold">Что осталось проблемным: {scenario.nCrit} показателей ниже 40</summary>
        {scenario.nCrit === 0 ? <p className="mt-3 text-sm">Показателей ниже критического порога не осталось.</p> : <ul className="mt-3 space-y-2 text-sm">
          {scenario.districts.flatMap((district) => district.indicators.filter((i) => i.critical).map((i) => <li key={`${district.districtId}-${i.indicator}`}>
            <strong>{district.name}</strong> — {INDICATORS.find((definition) => definition.code === i.indicator)?.name ?? i.indicator}: {i.final.toFixed(2)} / 100 (было {i.base.toFixed(2)}).
          </li>))}
        </ul>}
      </details>
